...
class DenseNetScratch(nn.Module):
    def __init__(self, num_classes=10, growth_rate=12, block_config=(3, 6, 12)):
        super().__init__()
        # 初始卷积
...
def plot_metrics(history):
    ...
    plt.xlabel('Epoch')
    plt.legend()
    plt.savefig('densenet_metrics1.png')
    plt.show()
...
def main():
    ...
    # 替换分类头（CIFAR-10需要10类输出）
    model_finetune.classifier = nn.Linear(model_finetune.classifier.in_features, 10)
