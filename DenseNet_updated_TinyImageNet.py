...
class DenseNetScratch(nn.Module):
    def __init__(self, num_classes=200, init_channels=64, growth_rate=32, blocks=[6, 12, 24, 16]):
        super().__init__()
        bn_size = 4
        drop_rate = 0.2
...
def plot_metrics(history):
    ...
    plt.xlabel('Epoch')
    plt.legend()
    plt.savefig('densenet_metrics3.png')
    plt.show()
...
def main():
    ...
    model_scratch = DenseNetScratch(num_classes=200, init_channels=64, growth_rate=32, blocks=[6, 12, 24, 16]).to(device)
